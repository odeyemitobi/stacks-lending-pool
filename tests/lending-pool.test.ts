...
describe("Lending Pool Tests", () => {
  ...
  it("Borrower can borrow, gets liquidated if price drops below liquidation threshold", () => {
    ...
    const liquidatorInitialBalance = getSBTCBalance(liquidator);
    ...
    expect(getSBTCBalance(liquidator)).toBe(liquidatorInitialBalance + 100);

    // Check that lender has earned yield from liquidation
    const lenderPendingYield = getPendingYield(lender);
    expect(lenderPendingYield).toBeGreaterThan(0);
  });
});
